...
v7 changes vs v6:
- Auto-detects latest SFT checkpoint (no hardcoded checkpoint-900)
- Default version tag: v7
- LoRA rank: r=64, alpha=128 (vs r=16, alpha=16 in v6)

Two-step merge pipeline:
1. Load Qwen2.5-7B-Instruct in fp16 (CPU)
2. Apply SFT LoRA (latest checkpoint) -> merge_and_unload()
...
Usage:
    python scripts/merge_sft_dpo_v7.py
    python scripts/merge_sft_dpo_v7.py --version v7
    python scripts/merge_sft_dpo_v7.py --skip-deploy
"""
...
def find_latest_sft_checkpoint():
    """Auto-detect the latest SFT checkpoint with an adapter."""
    sft_base = MODELS_DIR / 'chiro-no'
    if not sft_base.exists():
        print(f"  ERROR: SFT base dir not found: {sft_base}")
        return None

    checkpoints = sorted(
        sft_base.glob('checkpoint-*'),
        key=lambda p: int(p.name.split('-')[1])
    )

    # Find latest checkpoint with adapter_model.safetensors
    for ckpt in reversed(checkpoints):
        if (ckpt / 'adapter_model.safetensors').exists():
            return ckpt

    # Fallback: check base dir itself
    if (sft_base / 'adapter_model.safetensors').exists():
        return sft_base

    print(f"  ERROR: No SFT checkpoint with adapter_model.safetensors found in {sft_base}")
    return None
...
def step1_merge(merged_dir, version):
    ...
    # Auto-detect SFT checkpoint
    sft_dir = find_latest_sft_checkpoint()
    if sft_dir is None:
        return False
...
def step3_deploy_ollama(gguf_path, model_name, version):
    ...
    lines = [
        ...
        f'# Base: {BASE_MODEL}',
        f'# SFT: {version} LoRA r=64/alpha=128, 3 epochs, ~5,144 examples',
        f'# DPO: {version} DPO (1 epoch, ~828 pairs, beta=0.1)',
...
def main():
    parser = argparse.ArgumentParser(description='Merge SFT+DPO LoRAs and deploy to Ollama (v7)')
    parser.add_argument('--version', default='v7', help='Version tag (default: v7)')
