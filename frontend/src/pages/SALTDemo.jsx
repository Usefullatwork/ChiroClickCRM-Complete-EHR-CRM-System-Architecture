...
import { Globe, FileText, Copy, Clock, User, Calendar, RotateCcw } from 'lucide-react';
...
const MOCK_ENCOUNTERS = [
  {
    ...
    subjective: {
      chief_complaint: 'Patient presents with lower back pain radiating to the left leg. Pain rated 6/10. Onset was gradual, approximately 2 weeks ago. Pain is worse with prolonged sitting and bending forward.',
    ...
    objective: {
      ...
      palpation: 'Tenderness at L4-L5 and L5-S1 segments. Paravertebral muscle spasm noted bilaterally.'
    },
    ...
    plan: {
      treatment: 'Diversified lumbar adjustment L4-L5, L5-S1. Flexion-distraction 10 minutes. Muscle stimulation 15 minutes.',
  ...
  },
  {
    ...
    subjective: {
      chief_complaint: 'Initial evaluation for lower back pain. Pain rated 7/10. Difficulty with daily activities.',
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-2">
                    {lang === 'no' ? 'Full SALT-knapp med alternativer' : 'Full SALT Button with options'}
                  </label>
                  <SALTButton
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-2">
                    {lang === 'no' ? 'Hurtig SALT-linje med forhåndsinnstilte modifikatorer' : 'Quick SALT bar with preset modifiers'}
                  </label>
                  <SALTQuickBar
                    previousEncounter={MOCK_ENCOUNTERS[0]}
                    onApply={handleApply}
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg min-h-[80px]">
                    {currentNote.subjective?.chief_complaint ? (
                      ...
                      <p className="text-sm text-gray-400 italic">
                        {lang === 'no' ? 'Ingen data - bruk SALT for å klone' : 'No data - use SALT to clone'}
                      </p>
                    )}
                  </div>
                </div>
                    ...
                    <div className="p-3 bg-gray-50 rounded-lg">
                      <div className="flex flex-wrap gap-2">
                        {currentNote.treatments_selected.map((t, i) => (
                          <span key={i} className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded-lg">
                            {t.replace(/_/g, ' ')}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="p-3 bg-gray-50 rounded-lg">
                      <div className="flex flex-wrap gap-2">
                        {currentNote.icpc_codes?.map((code, i) => (
                          <span key={`icpc-${i}`} className="px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded-lg">
                            ICPC: {code}
                          </span>
                        ))}
                        {currentNote.icd10_codes?.map((code, i) => (
                          <span key={`icd-${i}`} className="px-2 py-1 text-xs bg-indigo-100 text-indigo-700 rounded-lg">
                            ICD-10: {code}
                          </span>
                        ))}
                      </div>
