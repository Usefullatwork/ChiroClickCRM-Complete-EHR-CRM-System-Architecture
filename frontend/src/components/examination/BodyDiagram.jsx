...
const MARKER_TYPES = {
  ...
  sharp_pain: { label: 'Skarp smerte', labelEn: 'Sharp Pain', color: '#dc2626', symbol: '✦', category: 'pain' },
  dull_pain: { label: 'Dump smerte', labelEn: 'Dull Pain', color: '#f87171', symbol: '◆', category: 'pain' },
  burning: { label: 'Brennende', labelEn: 'Burning', color: '#ea580c', symbol: '🔥', category: 'pain' },
  tp: { label: 'Triggerpunkt (TP)', labelEn: 'Trigger Point', color: '#f97316', symbol: 'TP', category: 'tissue' },
  tp_active: { label: 'Aktiv TP', labelEn: 'Active TP', color: '#c2410c', symbol: 'TA', category: 'tissue' },
  tp_latent: { label: 'Latent TP', labelEn: 'Latent TP', color: '#fb923c', symbol: 'TL', category: 'tissue' },
  lg: { label: 'Ligament (LG)', labelEn: 'Ligament', color: '#eab308', symbol: 'LG', category: 'tissue' },
  tn: { label: 'Sene (TN)', labelEn: 'Tendon', color: '#22c55e', symbol: 'TN', category: 'tissue' },
  tendinopathy: { label: 'Tendinopati', labelEn: 'Tendinopathy', color: '#16a34a', symbol: 'TD', category: 'tissue' },
  sk: { label: 'Hud (SK)', labelEn: 'Skin', color: '#3b82f6', symbol: 'SK', category: 'tissue' },
  fs: { label: 'Fascial (FS)', labelEn: 'Fascial', color: '#8b5cf6', symbol: 'FS', category: 'tissue' },
  adhesion: { label: 'Adhesjon', labelEn: 'Adhesion', color: '#7c3aed', symbol: 'AD', category: 'tissue' },
  numbness: { label: 'Nummenhet', labelEn: 'Numbness', color: '#64748b', symbol: '○', category: 'neuro' },
  tingling: { label: 'Prikking', labelEn: 'Tingling', color: '#475569', symbol: '∿', category: 'neuro' },
  weakness: { label: 'Svakhet', labelEn: 'Weakness', color: '#334155', symbol: 'W', category: 'neuro' },
  radiation: { label: 'Utstråling', labelEn: 'Radiation', color: '#ec4899', symbol: '→', category: 'referral' },
  referred: { label: 'Referert smerte', labelEn: 'Referred Pain', color: '#db2777', symbol: 'R', category: 'referral' },
  spasm: { label: 'Spasme', labelEn: 'Spasm', color: '#0891b2', symbol: 'SP', category: 'muscle' },
  hypertonicity: { label: 'Hypertonus', labelEn: 'Hypertonicity', color: '#0e7490', symbol: 'HT', category: 'muscle' },
  atrophy: { label: 'Atrofi', labelEn: 'Atrophy', color: '#155e75', symbol: 'AT', category: 'muscle' },
  swelling: { label: 'Hevelse', labelEn: 'Swelling', color: '#0284c7', symbol: 'SW', category: 'other' },
  warmth: { label: 'Varme', labelEn: 'Warmth', color: '#f59e0b', symbol: '☀', category: 'other' },
  crepitus: { label: 'Krepitasjon', labelEn: 'Crepitus', color: '#71717a', symbol: 'CR', category: 'other' }
};
 ...
 */
function BodyOutlinePosterior({ width = 200, height = 400 }) {
  return (
    <g>
      {/* Head */}
      <ellipse cx={width/2} cy={30} rx={25} ry={30} fill="none" stroke="#9ca3af" strokeWidth="1.5" />

      {/* Neck */}
      <rect x={width/2 - 12} y={55} width={24} height={20} fill="none" stroke="#9ca3af" strokeWidth="1.5" rx="5" />
      ...
      {/* Spine line */}
      <line x1={width/2} y1={75} x2={width/2} y2={200} stroke="#d1d5db" strokeWidth="1" strokeDasharray="4,2" />

      {/* Spine segments */}
      {[...Array(7)].map((_, i) => (
        <g key={`c${i}`}>
          <rect x={width/2 - 8} y={78 + i * 8} width={16} height={6} fill="none" stroke="#d1d5db" strokeWidth="0.5" rx="2" />
          <text x={width/2 - 18} y={84 + i * 8} fontSize="6" fill="#9ca3af">C{i + 1}</text>
        </g>
      ))}
      {[...Array(12)].map((_, i) => (
        <g key={`t${i}`}>
          <rect x={width/2 - 8} y={134 + i * 5} width={16} height={4} fill="none" stroke="#d1d5db" strokeWidth="0.5" rx="1" />
          <text x={width/2 + 12} y={138 + i * 5} fontSize="5" fill="#9ca3af">T{i + 1}</text>
        </g>
      ))}
      ...
      {[...Array(5)].map((_, i) => (
        <g key={`l${i}`}>
          <rect x={width/2 - 10} y={195 - i * 6} width={20} height={5} fill="none" stroke="#d1d5db" strokeWidth="0.5" rx="2" />
          <text x={width/2 - 22} y={199 - i * 6} fontSize="6" fill="#9ca3af">L{5-i}</text>
        </g>
      ))}
      ...
      <path
        ...
        strokeWidth="0.5"
      />
      <text x={width/2 - 22} y={210} fontSize="6" fill="#9ca3af">SAC</text>
    </g>
  );
}
 ...
 */
function BodyOutlineAnterior({ width = 200, height = 400 }) {
  return (
    <g>
      {/* Head */}
      <ellipse cx={width/2} cy={30} rx={25} ry={30} fill="none" stroke="#9ca3af" strokeWidth="1.5" />
      {/* Face features */}
      <ellipse cx={width/2 - 8} cy={25} rx={3} ry={2} fill="none" stroke="#d1d5db" strokeWidth="0.5" />
      <ellipse cx={width/2 + 8} cy={25} rx={3} ry={2} fill="none" stroke="#d1d5db" strokeWidth="0.5" />
      <path d={`M ${width/2 - 4} 35 Q ${width/2} 38, ${width/2 + 4} 35`} fill="none" stroke="#d1d5db" strokeWidth="0.5" />
      ...
      {/* Clavicles */}
      <path d={`M ${width/2 - 10} 78 L ${width/2 - 40} 85`} stroke="#d1d5db" strokeWidth="0.5" />
      <path d={`M ${width/2 + 10} 78 L ${width/2 + 40} 85`} stroke="#d1d5db" strokeWidth="0.5" />
      ...
      />
      {/* Left hand */}
      <ellipse cx={width/2 - 72} cy={190} rx={8} ry={12} fill="none" stroke="#9ca3af" strokeWidth="1" />
      ...
      />
      {/* Left knee */}
      <ellipse cx={width/2 - 25} cy={290} rx={8} ry={10} fill="none" stroke="#d1d5db" strokeWidth="0.5" />
      ...
      {/* Labels */}
      <text x={width/2 - 35} y={100} fontSize="5" fill="#9ca3af">Skulder V</text>
      <text x={width/2 + 20} y={100} fontSize="5" fill="#9ca3af">Skulder H</text>
    </g>
  );
}
    ...
    >
      ...
      {selected && (
        <g onClick={(e) => { e.stopPropagation(); onRemove(marker.id); }}>
          ...
          <X x={marker.x + size} y={marker.y - size - 8} width={8} height={8} stroke="white" strokeWidth={2} />
        </g>
      )}
    </g>
  );
...
export default function BodyDiagram({
  ...
  const categoryMarkers = Object.entries(MARKER_TYPES).filter(([_, type]) => type.category === selectedCategory);
            ...
            {categoryMarkers.map(([key, type]) => (
              <button
                ...
                className={`w-full text-left px-2 py-1.5 text-xs rounded-md flex items-center gap-2
                           transition-colors ${selectedType === key
            ...
            {markers.filter(m => m.vas).length > 0 && (
              <div className="mt-2 pt-1 border-t border-dashed">
                ...
                {(() => {
                  ...
                  const avgVas = painMarkers.reduce((sum, m) => sum + m.vas, 0) / painMarkers.length;
