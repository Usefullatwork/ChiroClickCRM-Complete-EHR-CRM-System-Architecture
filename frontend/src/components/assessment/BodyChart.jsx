...
export default function BodyChart({
  ...
  const [_selectedItem, _setSelectedItem] = useState(null);
  ...
  const handleMouseMove = useCallback(
    (event) => {
      if (!isDrawing || currentTool !== 'pencil' || !currentPath) {
        return;
      }
          ...
          {Object.entries(BODY_VIEWS).map(([key, view]) => (
            <button
              ...
              className={`px-2 py-1 text-xs rounded transition-colors ${
                currentView === key ? 'bg-blue-600 text-white' : 'bg-white border hover:bg-gray-50'
              }`}
            >
              {view.icon} {view.name}
            </button>
            ...
            {BRUSH_SIZES.map((bs) => (
              <button
                ...
                className={`w-6 h-6 rounded flex items-center justify-center transition-colors ${
                  brushSize === bs.size ? 'bg-blue-600' : 'bg-white border hover:bg-gray-100'
                }`}
                title={bs.name}
              >
                <div
...
export function BodyChartCompact({ markers = [], view = 'front', onClick }) {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <svg viewBox={viewData.viewBox} className="w-full h-32">
        <path d={viewData.outline} fill="none" stroke="#ccc" strokeWidth="1.5" />
        {viewMarkers.map((marker) => (
          <g key={marker.id}>
...
export function BodyChartGallery({ markers = [], annotations = [], onViewSelect }) {
  return (
        ...
        return (
          <div
            ...
            className={`bg-white rounded border p-2 cursor-pointer transition-all ${
              hasContent ? 'border-blue-400 bg-blue-50' : 'hover:border-gray-400'
            }`}
          >
            <svg viewBox={view.viewBox} className="w-full h-20">
              ...
              {viewMarkers.slice(0, 3).map((marker) => (
                <circle key={marker.id} cx={marker.x} cy={marker.y} r="6" fill={marker.color} />
              ))}
            </svg>
            <p className="text-xs text-center mt-1">
              {view.icon} {view.name}
              {hasContent && <span className="ml-1 text-blue-600">({viewMarkers.length})</span>}
            </p>
          </div>
        );
      })}
