  ...
  },
  {
    ...
    keywords: {
      ...
      no: [
        ...
        'bilateral beinnummenhet',
        'svakhet i begge bein',
        'nummenhet i begge bein',
        'progressiv bilateral',
      ],
    },
    description: {
  ...
  },
  {
    ...
    keywords: {
      en: [
        ...
        'shock down spine',
        'electric sensation flexion',
      ],
      no: [
        'lhermitte',
        'lhermittes tegn',
        'elektrisk støt ryggrad',
        'støt nedover ryggen',
        'elektrisk følelse fleksjon',
      ],
    },
    description: {
...
export function screenText(text, lang = 'en') {
  ...
  RED_FLAG_DEFINITIONS.forEach((flag) => {
    ...
    const found = keywords.some((keyword) => normalizedText.includes(keyword.toLowerCase()));
...
export function screenAge(age, lang = 'en') {
  ...
  RED_FLAG_DEFINITIONS.forEach((flag) => {
    if (flag.ageThreshold) {
      ...
      const matches = direction === 'over' ? age >= flag.ageThreshold : age < flag.ageThreshold;
...
export function screenExaminationFindings(findings, lang = 'en') {
  ...
  // Check for clonus
  if (
    findings.clonus_left ||
    findings.clonus_right ||
    findings.clonus === 'positive' ||
    findings.clonus === true
  ) {
    const flag = RED_FLAG_DEFINITIONS.find((f) => f.id === 'cord_clonus');
    if (flag && !detectedFlags.find((f) => f.id === 'cord_clonus')) {
      detectedFlags.push({
  ...
  if (findings.sensory) {
    ...
    const hasSaddleLoss = saddleAreas.some(
      (area) => findings.sensory[area] === 'absent' || findings.sensory[area] === 'decreased'
    );
    if (hasSaddleLoss) {
