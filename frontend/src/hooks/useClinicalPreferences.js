...
export const NOTATION_METHODS = [
  ...
  { id: 'gonstead_listing', name: { no: 'Gonstead Listing', en: 'Gonstead Listing' }, isVisual: false },
  ...
  { id: 'anatomical_chart', name: { no: 'Anatomisk Kart', en: 'Anatomical Chart' }, isVisual: true },
  { id: 'activator_protocol', name: { no: 'Activator Protokoll', en: 'Activator Protocol' }, isVisual: true },
...
export const useClinicalPreferences = () => {
  ...
  const getNotationName = useCallback((lang = preferences.language) => {
  ...
  const getNotationMethod = useCallback(() => {
    return NOTATION_METHODS.find(m => m.id === preferences.adjustmentNotation) || NOTATION_METHODS[0];
  }, [preferences.adjustmentNotation]);

  // Computed values
  const currentNotationMethod = NOTATION_METHODS.find(m => m.id === preferences.adjustmentNotation) || NOTATION_METHODS[0];
