-- Migration: 011_ai_feedback.sql
-- Description: Create AI feedback tables for continuous learning
-- Created: 2026-01-16
...
CREATE TABLE IF NOT EXISTS ai_feedback (
  ...
  template_id UUID,
  context_data JSONB,
  -- context_data example:
  -- {
  --   "chief_complaint": "neck pain",
  --   "patient_age": 45,
  --   "diagnosis_codes": ["L01", "L83"],
...
CREATE INDEX idx_ai_feedback_correction_type ON ai_feedback(correction_type);
CREATE INDEX idx_ai_feedback_encounter ON ai_feedback(encounter_id);

-- Composite index for analytics queries
CREATE INDEX idx_ai_feedback_analytics ON ai_feedback(suggestion_type, created_at, accepted);
...
CREATE INDEX idx_ai_metrics_org_date ON ai_daily_metrics(organization_id, date);
CREATE INDEX idx_ai_metrics_type ON ai_daily_metrics(suggestion_type);
...
CREATE TABLE IF NOT EXISTS ai_retraining_events (
  ...
  -- Types: 'threshold_reached', 'scheduled', 'manual', 'quality_drop'

  trigger_reason TEXT,
  suggestion_types_affected TEXT[], -- Array of affected types
  ...
  --   "test_samples": 100,
  --   "accuracy": 0.89,
  --   "acceptance_rate_before": 0.75,
  --   "acceptance_rate_after": 0.82
  -- }
...
CREATE INDEX idx_retraining_org ON ai_retraining_events(organization_id);
CREATE INDEX idx_retraining_status ON ai_retraining_events(status);
CREATE INDEX idx_retraining_created ON ai_retraining_events(created_at);
...
CREATE TABLE IF NOT EXISTS ai_model_versions (
  ...
  base_model VARCHAR(100), -- e.g., 'llama3.2', 'mistral'
...
CREATE INDEX idx_model_versions_org ON ai_model_versions(organization_id);
CREATE INDEX idx_model_versions_active ON ai_model_versions(is_active);
CREATE UNIQUE INDEX idx_model_versions_default ON ai_model_versions(organization_id) WHERE is_default = true;
...
CREATE TABLE IF NOT EXISTS communication_queue (
  ...
  communication_type VARCHAR(20) NOT NULL, -- 'SMS', 'EMAIL'
  template_id UUID REFERENCES message_templates(id) ON DELETE SET NULL,

  -- Content
  subject TEXT,
  content TEXT NOT NULL,
  personalized_content TEXT, -- After variable substitution
  ...
  -- Metadata
  batch_id UUID, -- For grouping bulk sends
...
CREATE INDEX idx_comm_queue_status ON communication_queue(status);
CREATE INDEX idx_comm_queue_scheduled ON communication_queue(scheduled_at) WHERE status = 'pending';
CREATE INDEX idx_comm_queue_patient ON communication_queue(patient_id);
CREATE INDEX idx_comm_queue_batch ON communication_queue(batch_id);
...
CREATE TABLE IF NOT EXISTS automation_workflows (
  ...
  trigger_type VARCHAR(50) NOT NULL,
  -- Types: 'PATIENT_CREATED', 'APPOINTMENT_SCHEDULED', 'APPOINTMENT_COMPLETED',
  --        'APPOINTMENT_MISSED', 'DAYS_SINCE_VISIT', 'BIRTHDAY', 'CUSTOM'

  trigger_config JSONB,
  -- Examples:
  -- { "days_since_visit": 90, "patient_status": ["ACTIVE"] }
  -- { "days_before_birthday": 0 }
  -- { "appointment_status": "NO_SHOW" }

  -- Condition filters
  conditions JSONB,
  -- { "patient_status": ["ACTIVE"], "has_email": true, "category": ["OSLO"] }

  -- Actions
  actions JSONB NOT NULL,
  -- [
  --   { "type": "SEND_SMS", "template_id": "...", "delay_minutes": 0 },
  --   { "type": "SEND_EMAIL", "template_id": "...", "delay_minutes": 1440 },
  --   { "type": "CREATE_FOLLOW_UP", "follow_up_type": "RECALL_3M" }
  -- ]

  -- Scheduling
  run_at_time TIME, -- Only run at specific time of day
  run_days INTEGER[], -- Days of week (0=Sunday)
  timezone VARCHAR(50) DEFAULT 'Europe/Oslo',

  -- Limits
  max_per_day INTEGER DEFAULT 100,
  min_interval_hours INTEGER DEFAULT 24, -- Minimum time between sends to same patient
...
CREATE INDEX idx_automation_org ON automation_workflows(organization_id);
CREATE INDEX idx_automation_active ON automation_workflows(is_active);
CREATE INDEX idx_automation_trigger ON automation_workflows(trigger_type);
...
CREATE TABLE IF NOT EXISTS automation_execution_log (
  ...
  -- [{ "type": "SEND_SMS", "status": "sent", "communication_id": "..." }]
...
CREATE INDEX idx_auto_log_workflow ON automation_execution_log(workflow_id);
CREATE INDEX idx_auto_log_patient ON automation_execution_log(patient_id);
CREATE INDEX idx_auto_log_executed ON automation_execution_log(executed_at);

-- =====================================================
-- FUNCTION: Update daily AI metrics
-- Called by cron job to aggregate feedback
-- =====================================================
...
-- Returns suggestion types that need retraining
-- =====================================================
...
CREATE TRIGGER update_ai_daily_metrics_updated_at
  BEFORE UPDATE ON ai_daily_metrics
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_retraining_events_updated_at
  BEFORE UPDATE ON ai_retraining_events
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_comm_queue_updated_at
...
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO chiroclickcrm_app;
-- GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO chiroclickcrm_app;
