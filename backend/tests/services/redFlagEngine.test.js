...
import redFlagEngine from '../../src/services/redFlagEngine.js';
...
describe('Red Flag Engine', () => {
  ...
  describe('scanForRedFlags', () => {
    ...
    test('should detect cauda equina bladder symptoms', () => {
      const flags = redFlagEngine.scanForRedFlags('Patient has bladder dysfunction and urinary incontinence');
    ...
    test('should detect Norwegian cauda equina symptoms', () => {
      const flags = redFlagEngine.scanForRedFlags('Pasient med blæreforstyrrelser og tap av kontroll over tarm');
    ...
    test('should detect immunocompromised status', () => {
      const flags = redFlagEngine.scanForRedFlags('Patient is immunocompromised due to chemotherapy');
    ...
    test('should detect inflammatory morning stiffness', () => {
      const flags = redFlagEngine.scanForRedFlags('Morning stiffness lasting more than 30 minutes that improves with activity');
    ...
    test('should return empty for normal content', () => {
      const flags = redFlagEngine.scanForRedFlags('Mild neck pain after desk work. Normal examination.');

      expect(flags.length).toBe(0);
    });

    test('should sort by severity', () => {
      const flags = redFlagEngine.scanForRedFlags('Bladder dysfunction, unexplained weight loss, and morning stiffness');

      // Critical should be first
      if (flags.length >= 2) {
        const severityOrder = { CRITICAL: 0, HIGH: 1, MODERATE: 2, LOW: 3 };
        for (let i = 1; i < flags.length; i++) {
          expect(severityOrder[flags[i - 1].severity]).toBeLessThanOrEqual(severityOrder[flags[i].severity]);
        }
      }
    });
  ...
  describe('calculateRiskScore', () => {
    ...
    test('should increase score for multiple flags', () => {
      ...
      const multipleFlags = redFlagEngine.scanForRedFlags('Fever and immunocompromised and recent surgery');
