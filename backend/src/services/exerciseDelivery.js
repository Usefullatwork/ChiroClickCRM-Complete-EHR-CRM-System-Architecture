...
export const generatePrescriptionPDF = async (organizationId, prescriptionId) => {
  try {
    const prescription = await exerciseLibraryService.getPrescriptionById(organizationId, prescriptionId)
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(10)
         ...
         .text(`Tlf: ${organization.phone || ''} | E-post: ${organization.email || ''}`, { align: 'center' })
      ...
      if (prescription.patient_instructions) {
        ...
        doc.font('Helvetica')
           .text(prescription.patient_instructions)
      }
      ...
      if (prescription.exercises && prescription.exercises.length > 0) {
        prescription.exercises.forEach((prescribedExercise, index) => {
          ...
          doc.fontSize(9)
             .font('Helvetica')
             .fillColor('#666666')
             .text(`${exercise.category} | ${exercise.difficultyLevel === 'beginner' ? 'Nybegynner' : exercise.difficultyLevel === 'intermediate' ? 'Middels' : 'Avansert'}`)
             .fillColor('#000000')
          ...
          // Parameters box
          const params = []
          if (prescribedExercise.sets) params.push(`${prescribedExercise.sets} sett`)
          if (prescribedExercise.reps) params.push(`${prescribedExercise.reps} repetisjoner`)
          if (prescribedExercise.holdSeconds) params.push(`Hold ${prescribedExercise.holdSeconds} sek`)
          if (prescribedExercise.frequencyPerDay) params.push(`${prescribedExercise.frequencyPerDay}x daglig`)
      ...
      doc.fontSize(8)
         .font('Helvetica')
         .fillColor('#666666')
         .text('Dette øvelsesprogrammet er tilpasset deg basert på din undersøkelse. Stopp øvelsene hvis du opplever økt smerte og kontakt klinikken. Programmet er gyldig i 90 dager.', {
           align: 'center'
         })
         .text('This exercise program is customized based on your examination. Stop exercises if you experience increased pain and contact the clinic. Valid for 90 days.', {
           align: 'center'
         })
 ...
 */
export const sendPrescriptionEmail = async (organizationId, prescriptionId, options = {}) => {
  try {
    const prescription = await exerciseLibraryService.getPrescriptionById(organizationId, prescriptionId)
