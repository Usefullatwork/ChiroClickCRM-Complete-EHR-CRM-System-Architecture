...
class MacroService {
  ...
  async getMacroMatrix(organizationId) {
    ...
    logger.debug('Macro matrix loaded', {
      organizationId,
      duration,
      macroCount: result.rows.length,
    });
   ...
   * Update a macro
   */
  async updateMacro(organizationId, macroId, macroData) {
    const fields = [];
    const values = [organizationId, macroId];
    let paramIndex = 3;

    if (macroData.name !== undefined) {
      fields.push(`macro_name = $${paramIndex++}`);
      values.push(macroData.name);
    }
    if (macroData.text !== undefined) {
      fields.push(`macro_text = $${paramIndex++}`);
      values.push(macroData.text);
    }
    if (macroData.category !== undefined) {
      fields.push(`category = $${paramIndex++}`);
      values.push(macroData.category);
    }
    if (macroData.subcategory !== undefined) {
      fields.push(`subcategory = $${paramIndex++}`);
      values.push(macroData.subcategory);
    }
    if (macroData.shortcutKey !== undefined) {
      fields.push(`shortcut_key = $${paramIndex++}`);
      values.push(macroData.shortcutKey);
    }
    if (macroData.soapSection !== undefined) {
      fields.push(`soap_section = $${paramIndex++}`);
      values.push(macroData.soapSection);
    }
    ...
    const result = await query(
      `UPDATE clinical_macros SET ${fields.join(', ')} WHERE organization_id = $1 AND id = $2 RETURNING *`,
      values
    );
   ...
   * Soft-delete a macro
   */
  async deleteMacro(organizationId, macroId) {
    const result = await query(
      `UPDATE clinical_macros SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE organization_id = $1 AND id = $2 RETURNING id`,
      [organizationId, macroId]
    );
