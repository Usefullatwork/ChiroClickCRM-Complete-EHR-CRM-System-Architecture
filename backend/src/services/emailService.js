...
export const sendExerciseProgramEmail = async (params) => {
  ...
  const exercises = prescription.exercises?.map((ex, index) => ({
    number: index + 1,
    name: ex.exercise?.nameNorwegian || ex.exercise?.name || 'Ovelse',
    thumbnailUrl: ex.exercise?.thumbnailUrl || `${config.tracking.baseUrl}/images/exercise-placeholder.png`,
    sets: ex.sets || '-',
    reps: ex.reps || '-',
  ...
  const variables = {
    ...
    patientFullName: `${patient.firstName || patient.first_name || ''} ${patient.lastName || patient.last_name || ''}`.trim(),
    clinicName: organization.name || 'Klinikken',
    clinicLogo: organization.logoUrl || organization.logo_url || `${config.tracking.baseUrl}/images/logo.png`,
    ...
    prescriptionDate: new Date(prescription.prescribed_at || Date.now()).toLocaleDateString('nb-NO'),
...
export const sendAppointmentReminderEmail = async (params) => {
  ...
  const variables = {
    ...
    providerName: provider ? `${provider.firstName || provider.first_name || ''} ${provider.lastName || provider.last_name || ''}`.trim() : '',
...
export const trackOpen = async (emailId) => {
  try {
    await query(
      `UPDATE communications SET opened_at = NOW() WHERE id = $1 AND opened_at IS NULL`,
      [emailId]
    )
  } catch (error) {
...
export const trackClick = async (emailId, linkUrl) => {
  try {
    await query(
      `UPDATE communications SET clicked_at = NOW() WHERE id = $1`,
      [emailId]
    )
