...
class SALTService {
    ...
    async cloneEncounter(patientId, providerId, options = {}) {
        ...
        const newEncounter = {
            ...
            assessment: sectionsToClone.assessment
                ...
                : null,
            plan: sectionsToClone.plan
                ? this.processSection(lastEncounter.plan, stripDates)
                : null,
    ...
    processSection(text, stripDates = true) {
        ...
        if (stripDates) {
            ...
            processed = processed.replace(new RegExp(`(${this.placeholder}\\s*)+`, 'g'), this.placeholder + ' ');
     ...
     */
    getWarnings(daysSince, lastEncounter) {
        const warnings = [];
