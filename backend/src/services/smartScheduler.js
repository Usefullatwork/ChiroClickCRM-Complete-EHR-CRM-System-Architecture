...
async function scheduleCommuncation({
  ...
  const result = await query(`
    ...
    RETURNING *
  `, [
    organizationId, patientId, communicationType, templateId,
    customMessage, scheduledDate, scheduledTime, triggerType,
    triggerAppointmentId, triggerDaysAfter, createdBy
  ])
...
async function scheduleFollowUpAfterVisit(appointmentId, organizationId) {
  ...
  if (apptResult.rows.length === 0) return null
  ...
  for (const rule of rulesResult.rows) {
    ...
    if (rule.condition_no_appointment_scheduled) {
      ...
      if (futureAppt.rows.length > 0) {
        logger.info(`Skipping follow-up for patient ${appointment.patient_id} - has future appointment`)
        continue
      }
    }
...
async function resolveDecision(decisionId, decision, userId, note = null) {
  ...
  switch (decision) {
    case 'extend':
      ...
      logger.info(`Extended scheduled communication ${decisionRecord.scheduled_communication_id} to ${decisionRecord.suggested_new_date}`)
      break
    ...
    case 'send_anyway':
      ...
      await query(`
        ...
        WHERE id = $2
      `, [userId, decisionRecord.scheduled_communication_id])
      logger.info(`Keeping original date for scheduled communication ${decisionRecord.scheduled_communication_id}`)
      break
  }
    ...
    try {
      ...
      if (futureAppt.rows.length > 0) {
        ...
        await query(`
          ...
          WHERE id = $1
        `, [comm.id])
        logger.info(`Auto-cancelled communication ${comm.id} - patient has appointment on ${futureAppt.rows[0].appointment_date}`)
        continue
      }
      ...
      if (comm.communication_type === 'sms' && comm.phone) {
        ...
        sent.push(comm.id)
        logger.info(`Sent scheduled ${comm.communication_type} to ${comm.first_name} ${comm.last_name}`)
      }
    } catch (error) {
...
async function importAppointments(organizationId, appointments, source = 'solvitjournal', userId) {
  ...
  for (const appt of appointments) {
    try {
      ...
      if (existingAppt.rows.length > 0) {
        ...
        `, [
          organizationId, patientId, appt.date, appt.time,
          appt.duration || 30, appt.status || 'scheduled',
          appt.visitType || 'behandling', appt.notes
        ])
        stats.created++
      }
    } catch (error) {
  ...
  `, [
    organizationId, source, stats.total, stats.created,
    stats.updated, stats.patientsCreated, stats.errors,
    JSON.stringify(stats.errorLog), userId
  ])
...
async function updateCommunicationRule(ruleId, updates) {
  ...
  const allowedFields = [
    'name', 'description', 'trigger_days', 'communication_type',
    'template_id', 'default_message', 'on_new_appointment',
    'extend_days', 'is_active', 'condition_no_appointment_scheduled'
  ]
