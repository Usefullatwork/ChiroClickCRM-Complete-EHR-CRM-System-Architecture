...
export const executeAction = async (organizationId, actionType, config, context = {}) => {
  const { patientId, _leadId, executionId } = context;
...
const executeSendSMS = async (organizationId, config, context) => {
  ...
  if (templateId) {
    const templateResult = await query(`SELECT body FROM message_templates WHERE id = $1`, [
      templateId,
    ]);
...
const executeCreateTask = async (organizationId, config, context) => {
  ...
  const patientResult = await query(`SELECT first_name, last_name FROM patients WHERE id = $1`, [
    patientId,
  ]);
