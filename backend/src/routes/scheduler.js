...
import {
  ...
  bulkResolveDecisionsSchema,
  updateRuleSchema,
  getPatientCommsSchema,
  cancelMessageSchema,
  sendMessagesSchema,
...
router.post(
  ...
  validate(scheduleCommunicationSchema),
  async (req, res) => {
    try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const { organizationId } = req;
      const result = await schedulerService.scheduleFollowUpAfterVisit(
        appointmentId,
        organizationId
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!['extend', 'cancel', 'send_anyway'].includes(decision)) {
        return res
          .status(400)
          .json({ error: 'Invalid decision. Use: extend, cancel, send_anyway' });
      }
...
router.post(
  '/decisions/bulk',
  requireRole(['ADMIN', 'PRACTITIONER']),
  validate(bulkResolveDecisionsSchema),
  async (req, res) => {
    try {
...
router.get(
  '/patient/:patientId',
  requireRole(['ADMIN', 'PRACTITIONER']),
  validate(getPatientCommsSchema),
  async (req, res) => {
    try {
...
router.post(
  '/import',
  requireRole(['ADMIN']),
  validate(importAppointmentsSchema),
  async (req, res) => {
    try {
      const { organizationId, user } = req;
