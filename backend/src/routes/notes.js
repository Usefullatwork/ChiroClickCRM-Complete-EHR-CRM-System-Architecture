...
router.get('/',
  ...
  async (req, res) => {
    try {
      const result = await clinicalNotesService.getAllNotes(
        req.organizationId,
        {
          page: req.query.page,
...
router.get('/templates',
  ...
  async (req, res) => {
    try {
      const templates = await clinicalNotesService.getNoteTemplates(
        req.organizationId,
        {
          templateType: req.query.templateType,
...
router.get('/drafts',
  ...
  async (req, res) => {
    try {
      const drafts = await clinicalNotesService.getUserDrafts(
        req.organizationId,
        req.userId
      );
...
router.get('/search',
  ...
  async (req, res) => {
    try {
      ...
      const notes = await clinicalNotesService.searchNotes(
        req.organizationId,
        q,
        { patientId, limit: parseInt(limit) || 20 }
      );
...
router.get('/:id',
  ...
  async (req, res) => {
    try {
      const note = await clinicalNotesService.getNoteById(
        req.organizationId,
        req.params.id
      );
...
router.get('/:id/history',
  ...
  async (req, res) => {
    try {
      const history = await clinicalNotesService.getNoteHistory(
        req.organizationId,
        req.params.id
      );
...
router.delete('/:id',
  ...
  async (req, res) => {
    try {
      await clinicalNotesService.deleteNote(
        req.organizationId,
        req.params.id,
        req.userId
      );
